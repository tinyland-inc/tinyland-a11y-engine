...
`;
  ...
  private generateExecutiveSummary(report: any): string {
    ...
    const status = summary.violations.critical > 0 ? '🚨 **CRITICAL**' : 
                  summary.failed > 0 ? '⚠️ **NEEDS ATTENTION**' : 
                  '✅ **PASSING**';
...
${summary.violations.critical > 0 ? 
  `> **⚠️ Critical Issues Found**: There are ${summary.violations.critical} critical accessibility violations that must be addressed immediately.` : 
  summary.failed > 0 ?
...
`;
  ...
  generatePRComment(results: AccessibilityTestResult[]): string {
    ...
    const status = summary.violations.critical > 0 ? '🚨' : 
                  summary.failed > 0 ? '⚠️' : '✅';
...
`;
      ...
      comment += `✅ **All accessibility tests passed!** Great job maintaining accessibility standards.\n`;
    }
